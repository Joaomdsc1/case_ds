...
# 5. CRIAÇÃO DO PIPELINE COM LightGBM
# Trocamos o RandomForestClassifier pelo LGBMClassifier
pipeline = Pipeline(steps=[('preprocessor', preprocessor),
                           ('smote', SMOTE(random_state=42)),
                           ('classifier', lgb.LGBMClassifier(random_state=42))])

# 6. DEFINIÇÃO DO GRID DE HIPERPARÂMETROS PARA O LightGBM
param_grid = {
...
# 7. CONFIGURAÇÃO E EXECUÇÃO DO GRIDSEARCHCV
grid_search = GridSearchCV(pipeline, param_grid, cv=5, n_jobs=-1, scoring='f1_weighted', verbose=2)

print("Iniciando a busca pelos melhores hiperparâmetros com LightGBM + SMOTE + GridSearchCV...")
grid_search.fit(X_train, y_train)

# 8. ANÁLISE DOS RESULTADOS
print("\n--- Resultados do GridSearchCV com LightGBM ---")
print("Melhores parâmetros encontrados:")
print(grid_search.best_params_)

# 9. AVALIAÇÃO DO MELHOR MODELO NO CONJUNTO DE TESTE
print("\n--- Avaliação Final do Melhor Modelo (com LightGBM) no Conjunto de Teste ---")
